...
from intake.codec import parse_payload
...
def phase2_accuracy() -> dict:
    ...
    rej_ok = sum(1 for b in bad if parse_payload(b) is None)
...
def phase1_accuracy() -> dict:
    ledger = RollingChronicleStore(max_nodes=1000)

    ok_accept = 0
    total_accept = 0
    prev = tx_fingerprint("ROOT")
    for i in range(2000):
        tid = tx_fingerprint(f"tx{i}")
        total_accept += 1
    ...
    missing_parent = tx_fingerprint("never_seen")
    b_ok, missing = ledger.add(tx_fingerprint("bad"), [missing_parent])
...
def phase4_accuracy() -> dict:
    ...
    for i in range(200):
        txid = f"tx{i}"
        p = intel_engine.make_proof(txid, vec, "v")
        total += 1
        ok += 1 if intel_engine.verify(p)[0] else 0

        # tamper
        total += 1
        bad = IntelProof(validator_id=p.validator_id, tx_id=p.tx_id, score=p.score + 1.0, proof=p.proof)
        ok += 1 if (not intel_engine.verify(bad)[0]) else 0
...
def phase5_accuracy() -> dict:
    ...
    for s in safe:
        _, issues = covenant_auditor.auto_patch(s)
        blocked = bool(issues)
        if blocked:
            fp += 1
        else:
...
def phase6_accuracy() -> dict:
    ...
    # normal: stable amounts; sentinel flags spikes or high-rate bursts.
    samples = []
    ...
    # high-rate burst (tiny timestamp increments)
    for i in range(50):
        t += 0.001
    ...
    for s, amt, ts, y in samples:
        r, _ = sentinel_scorer.score(s, amt, ts)
        scores.append(r)
        labels.append(y)
...
def phase8_accuracy() -> dict:
    want = {
        ...
        "/phase4/intel/prove",
        "/phase4/intel/verify",
        "/phase5/covenant/audit",
        "/phase6/sentinel/score",
        "/phase7/he/keygen",
        "/phase7/he/encrypt",
        "/phase7/he/decrypt",
        "/phase7/he/add",
...
def main() -> None:
    report = {
        ...
        "notes": {
            "sentinel_covenant_accuracy": "synthetic labeled data; real accuracy needs real labels",
