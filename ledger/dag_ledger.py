...
class DagLedger:
    ...
    def __init__(self) -> None:
        # tx_id -> parents tuple
        self._parents: Dict[str, Tuple[str, ...]] = {GENESIS: ()}
        # tx_id -> number of children (0 means tip)
        self._child_count: Dict[str, int] = {GENESIS: 0}
        self._tips: Set[str] = {GENESIS}
    ...
    def get_missing_parents(self, parents: Iterable[str]) -> List[str]:
        missing: List[str] = []
        for p in parents:
            if p != GENESIS and p not in self._parents:
                missing.append(p)
        return missing
    ...
    def add(self, tx_id: str, parents: List[str]) -> Tuple[bool, List[str]]:
        ...
        parent_tuple = tuple(parents) if parents else (GENESIS,)
...
class RollingDagLedger:
    """Bounded-memory ledger for very large streams.
    ...
    def __init__(self, max_nodes: int = 1_000_000) -> None:
        ...
        bits = int(os.getenv("LEDGER_BLOOM_BITS", str(512 * 1024 * 1024)))
    ...
    def add(self, tx: int, parents: List[int]) -> Tuple[bool, List[int]]:
        ...
        if not parents:
            parents = [tx_key(GENESIS)]

        # Best-effort missing detection (only within current window).
        missing: List[int] = []
        for p in parents:
            if p != tx_key(GENESIS) and p not in self._parents:
...
_MAX = int(os.getenv("LEDGER_MAX_NODES", "1000000"))
dag_ledger = RollingDagLedger(max_nodes=_MAX)
