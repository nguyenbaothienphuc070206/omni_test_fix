"""Phase 4: Intel proof engine (MVP).
...
class IntelProofEngine:
    def __init__(self, dim: int = 64, threshold: float = 0.0) -> None:
        self.dim = dim
        env_thr = os.getenv("INTEL_THRESHOLD")
    ...
    def make_proof(self, tx_id: str, vector: List[float], validator_id: str) -> IntelProof:
        ...
        return IntelProof(validator_id=validator_id, tx_id=tx_id, score=score, proof=proof)

    def verify(self, p: IntelProof) -> Tuple[bool, str]:
